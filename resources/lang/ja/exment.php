...
return [
    ...
    'dashboard' => [
        ...
        'filter_bar' => [
            'header' => 'ダッシュボードフィルター',
            'description' => 'ダッシュボード上部に、全ボックス共通の絞り込みフィルターバーを表示します。フィルター元テーブルの列をフィルター項目として設定します。同じ列名を持つテーブルを使用するボックスのみ絞り込まれます。',
            'source_table' => 'フィルター元テーブル',
            'dims' => 'フィルター項目',
            'dim_column' => '列',
            'dim_label' => '表示名',
            ...
            'check_header' => '適合チェック',
            'check_recommend' => '推奨のフィルター元テーブル：:table（:total 件中 :covered 件のボックスに適用されます）',
            'check_covered' => ':total 件中 :covered 件のボックスがこのフィルターで絞り込まれます',
            'check_box_ok' => '適用',
            'check_box_ng' => '対象外',
            'check_reason_columns' => 'このボックスのテーブルに同じ列名がありません',
            'check_box_mismatch' => '型不一致',
            'check_box_novalue' => '値不一致',
            'check_reason_mismatch' => '同じ列名ですが値の型が異なるため、絞り込むと0件になります：:columns',
            'check_reason_novalue' => '同じ列名ですが共通する値が1件もないため、絞り込むと0件になります（別の意味の列である可能性があります）：:columns',
            'check_recommend_better' => '選択中の「:selected」は :total 件中 :selected_covered 件のボックス（計 :selected_reach 列）に適用されます。「:best」なら :best_covered 件（計 :best_reach 列）に適用されます。選択されたテーブルをご確認ください。',
            'check_reason_type' => 'チャート以外のボックスはフィルターを適用しません',
            'check_pick_table' => 'フィルター元テーブルを選択すると、どのボックスが絞り込まれるかを表示します。',
            'check_no_boxes' => 'このダッシュボードには、データテーブルを持つボックスがまだありません。',
            'check_after_boxes' => 'ダッシュボードを保存してボックスを追加すると、適合チェックを表示します。',
            'check_all_missed' => 'どのボックスも絞り込まれません。フィルター元テーブルの列名が、ボックスのテーブルの列名と一致していない可能性があります。上の推奨テーブルをご確認ください。',
            'not_affected' => 'フィルター対象外',
            'drill_hint' => 'クリックで詳細へ:',
            'click_filter_hint' => 'クリックで絞り込み',
            'narrow_first' => '候補が多すぎます。上位のフィルターで絞り込むと選択できます',
            ...
            'detailed_filter' => '詳細フィルター条件',
            'cond_field' => '項目',
            'cond_condition' => '条件',
            'cond_value' => '値',
            'cond_value_placeholder' => '値を入力',
            'cond_add' => '条件を追加',
            'cond_apply' => '適用',
            'cond_clear' => '条件をクリア',
            'cond_op' => [
            ...
            'disabled_with' => '「:label」フィルター使用中は選択できません',
            // readability cap: the drilled depth had too many groups, chart stayed shallower
            // (:name = the breakdown that was too large, e.g. クラス別 平均点)
            'level_too_many' => '「:name」は項目が多すぎます。上位のフィルターで絞り込むと表示されます',
            'help' => [
                'source_table' => 'フィルターの選択肢を取得するテーブルです。選択して保存すると、フィルター項目を設定できるようになります。空欄にするとフィルターバーを表示しません。',
                'dims' => 'フィルターバーに表示する項目です。項目同士の親子関係（例：学年 › クラス › 児童）はExmentの参照設定（参照先マスタ同士の関係）から自動で判定します：親を変更すると子の選択は自動でクリアされ、階層はパンくずに表示されます。設定は不要です。',
                'dim_targets' => 'このフィルターを適用するチャートボックスを選択します。選択したボックスのみが絞り込まれ、それ以外のボックスは（同じ列を持っていても）影響を受けません。空欄の場合は従来どおり、同じ列名を持つすべてのボックスに適用されます。',
            ],
        ],
        'chart' => [
        ...
        'ai_summary' => 'AI要約',
        'ai_summary_help' => '有効にすると、グラフデータをAIサービスへ送信し、AIによる要約・分析を生成します。機密性の高いデータを扱うダッシュボードでは無効にしてください。無効の場合、AI機能は利用できません。',
        'ai_chat' => [
            // Client-side errors
            'error_generic'       => 'AIへの接続でエラーが発生しました。もう一度お試しください。',
            'error_connect'       => 'AIサービスに接続できません。',
            // Server / service messages
            'no_valid_answer'     => 'AIから有効な回答を取得できませんでした。もう一度お試しください。',
            'api_error_timeout'   => 'AIサービスへの接続が中断またはタイムアウトしました。もう一度お試しください。',
            'api_error_rate'      => 'AIの呼び出し回数の上限に達しました（クォータ／レート制限）。数分後にもう一度お試しください。',
            'api_error_auth'      => 'AIサービスのAPIキーが無効か、権限がありません。設定をご確認ください。',
            'api_error_bad_request' => 'AIサービスへのリクエストが不正です（モデルがこの機能に対応していない可能性があります）。',
            'api_error_upstream'  => 'AIサービスで問題が発生しています。しばらくしてからお試しください。',
            'api_error_unknown'   => 'AIサービスに接続できません。しばらくしてからお試しください。',
            'error_rate_limit'    => '1時間あたりのAIリクエスト上限（:limit回）を超えました。しばらくしてからお試しください。',
        ],
        'ai_insight' => [
            'toggle'     => 'AI要約',
            'generating' => '要約を生成中...',
            'regenerate' => '再生成',
            'withheld'   => '機微な列が含まれているため、このデータはAIに送信されませんでした。',
            'stat_highest' => '最大',
            'stat_lowest'  => '最小',
            'stat_average' => '平均',
            'stat_range'   => '幅',
            'anomaly_title'   => '異常値',
            'expected_range'  => '想定範囲',
            'stable'          => '異常値なし — すべて想定範囲内',
            'chart_render_error' => 'このグラフを表示できませんでした（拡張ライブラリが不足しているか、データが適していない可能性があります）。',
        ],
        ...
        'dashboard_box_options' => [
            ...
            'chart_filters_help' => 'チャートのツールバーに絞り込み用の項目を表示します（値を複数選べるチェックリスト。数値・日付の列は「から／まで」の範囲）。選択した列の値で「このチャートのみ」を絞り込みます（他のボックスには影響しません）。ダッシュボードフィルターとは独立して動作し、両方指定した場合は両方の条件で絞り込まれます。',
            'chart_sort_value' => '並び順',
            'chart_sort_value_desc' => '値の大きい順',
            'chart_sort_value_help' => 'グループを集計値の大きい順に並べます（未指定 = ビューの並び順）。',
            'chart_max_groups' => '表示グループ数（上位N件）',
            'chart_max_groups_help' => '並び替え後の先頭N件のグループだけを表示します（0 または空欄 = すべて）。「値の大きい順」と組み合わせると、上位10件などのランキングを通常のチャートとして表示できます。ダッシュボードフィルターやチャートフィルターにもそのまま連動します。',
        ...
        'message' => [
            ...
            'need_multiseries' => 'このグラフ種別は、2列でグループ化した集計ビュー（1列目＝横軸、2列目＝系列）と集計値が必要です。',
    ...
    'chart' => [
        ...
        'chart_type_options' => [
            ...
            'hbar' => '横棒グラフ (ECharts)',
            'area' => 'エリアグラフ (ECharts)',
            'doughnut' => 'ドーナツグラフ (ECharts)',
            'radar' => 'レーダーチャート (ECharts)',
            'funnel' => 'ファネルグラフ (ECharts)',
            'gauge' => 'ゲージ / KPI (ECharts)',
            'scatter' => '散布図 (ECharts)',
            'mbar' => 'グループ棒グラフ・複数系列 (ECharts)',
            'sbar' => '積み上げ棒グラフ・複数系列 (ECharts)',
            'mline' => '複数折れ線・複数系列 (ECharts)',
            'heatmap' => 'ヒートマップ・複数系列 (ECharts)',
            'sarea' => '積み上げエリア・複数系列 (ECharts)',
            'treemap' => 'ツリーマップ・複数系列 (ECharts)',
            'sunburst' => 'サンバースト・複数系列 (ECharts)',
            'boxplot' => '箱ひげ図・複数系列 (ECharts)',
        ],
