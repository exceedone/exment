...
return [
    ...
    'custom_table' => [
        ...
        'comment_flg' => 'コメント使用',
        'comment_link_relation' => 'コメントで言及されたデータを自動リンクする種別',
        ...
        'help' => [
            ...
            'comment_link_relation' => '英数字で種別名を入力すると（例：related）、コメント本文に含まれる他データの見出し（例：PORTAL-12）や #12 の形式から、自動で関連リンクを作成します。<br/>空の場合は何もしません。',
    ...
    'custom_column' => [
        ...
        'help' => [
            ...
            'grid_icon' => '値の前に表示するFont Awesomeのアイコン名です。例：fa-phone、fa-users。「アバター」以外のすべての表示形式で使用できます。',
            'grid_nowrap' => 'YESにすることで、一覧のセル内で文字を折り返さなくなります。',
            'grid_value_colors' => '選択肢ごとに色を変える場合に入力してください。1行につき「値,文字色」の形式です。背景色と枠線色も指定する場合は「値,文字色,背景色,枠線色」と入力してください。例：1,#c0392b<br/>バッジでは、指定した1色目が塗りつぶしの色になります。進捗バーでは「しきい値,色」の形式で、値がしきい値以上のときの色を指定します。例：75,#f39c12',
            ...
            'cross_link_relation' => '英数字で種別名を入力すると（例：related）、この列で選んだデータ側の詳細画面にも、逆向きのリンクが表示されます。<br/>空の場合は従来どおり片方向の参照です。',
    ...
    'custom_view' => [
        ...
        'description_custom_view_kanban_columns' => 'カードに表示する列を設定します。「表示位置」でカード内のどこに出すか、「表示スタイル」で見た目(タグ、ピル、色付きバッジなど)を指定できます。<br/>※「アイコン」には Font Awesome のクラス名(例: fa-phone)を入力してください。ピル・チップ・ポイントのスタイルで使用します。<br/>※設定しない場合、カードにはタイトルのみ表示されます。',
        ...
        'help' => [
            ...
            'kanban_done_keys' => '「完了」として扱うカンバンの列を、テーブルの選択肢から選びます。<br/>※選んだ列のデータは、期限のカウントを止め、集計バーの「完了」に数えます。<br/>※ワークフローから作る場合、「完了」と設定された状態は自動的に完了として扱うため、ここでの指定は不要です。',
            'kanban_title_column' => 'カード内で太字のタイトルとして表示する列です。<br/>※指定した場合、カード上部にデータの表示名がリンクとして表示されます。<br/>※指定しない場合、データの表示名がそのままタイトルになります。',
            'kanban_assignee_column' => '担当者を表す列です。<br/>※カード下部にアバター付きで表示され、未入力のカードには「未割り当て」が表示されます。<br/>※集計バーの「未割り当て」、フィルタの「未割り当てのみ」、一括変更の「担当一括変更」でも使用します。',
            'kanban_limit_column' => '期限(締切)の日付列です。<br/>※現在時刻との差から、カードに「残○h内」「まもなく期限」「期限超過」のバッジを表示します。<br/>※「完了とみなす値」のデータは「達成」と表示します。',
            ...
            'kanban_hide_keys' => '選んだ値の列をボードに表示しません。<br/>例：「完了」「取止め」を選ぶと、終わった仕事が消えて、進行中の仕事だけのボードになります。<br/>※非表示の列のデータは読み込みません。件数が多いテーブルでは、これだけで表示が大幅に速くなります。<br/>※合計・平均・KPIにも含まれません。<br/>※ワークフローの場合、非表示の状態へ進めるとカードはボードから消えます（データは残ります）。<br/>※「未設定」の列は非表示にできません。',
            ...
            'kanban_drawer' => 'YESにすると、カードをクリックしたときに右側から詳細パネルを開きます。<br/>※NOにすると、データの詳細画面へ移動します。',
            'kanban_position' => 'カード内のどこに表示するかを選びます。',
            'kanban_style' => 'カード内での見た目を選びます。',
        ...
        'message' => [
            ...
            'preview_not_supported' => 'このビュー種類はプレビューに対応していません。一覧ビューとカンバンビューでご利用いただけます。',
