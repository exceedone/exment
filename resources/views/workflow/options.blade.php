...
<label for="{{$id['comment']}}" class="{{$viewClass['label']}} control-label">{{$label}}</label>
    ...
    <div class="{{$viewClass['field']}}">
        ...
        <label for="{{$id['comment']}}" class="control-label small asterisk">コメント</label>
        ...
        <label for="{{$id['comment']}}" class="control-label small asterisk">次のステータスへ進む条件</label>
        ...
        <div class="radio icheck">
            <label>
                ...
                <input type="text" style="width:50px; text-align:right;" value="{{old($column['flowNextCount'], $value['flowNextCount']) ?? 1}}" />&nbsp;人以上
            </label>
        </div>
