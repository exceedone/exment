...
@section('content')
        <p class="login-box-msg text-center">{{ trans('admin.setting') }}(2/4) : {{exmtrans('install.database.header')}}</p>
        ...
        <form action="{{ admin_url('install') }}" method="post">
            ...
            <div class="form-group pt-3 {!! !$errors->has('connection') ?: 'has-error' !!}">

                @if($errors->has('connection')) @foreach($errors->get('connection') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.connection')}}:</label>
            ...
            <div class="form-group pt-3 {!! !$errors->has('host') ?: 'has-error' !!}">

                @if($errors->has('host')) @foreach($errors->get('host') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.host')}}:</label>
                <input type="text" class="form-control" name="host" value="{{ old('host', $host) }}" required />
            </div>

            <div class="form-group pt-3  {!! !$errors->has('port') ?: 'has-error' !!}">

                @if($errors->has('port')) @foreach($errors->get('port') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.port')}}:</label>
                <input type="text" class="form-control" name="port" value="{{ old('port', $port) }}" required />
            </div>

            <div class="form-group pt-3  {!! !$errors->has('database') ?: 'has-error' !!}">
                @if($errors->has('database')) @foreach($errors->get('database') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.database')}}:</label>
                <input type="text" class="form-control" name="database" value="{{ old('database', $database) }}" required />
            </div>

            <div class="form-group pt-3  {!! !$errors->has('username') ?: 'has-error' !!}">
                @if($errors->has('username')) @foreach($errors->get('username') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.username')}}:</label>
                <input type="text" class="form-control" name="username" value="{{ old('username', $username) }}" required />
            </div>

            <div class="form-group pt-3  {!! !$errors->has('password') ?: 'has-error' !!}">
                @if($errors->has('password')) @foreach($errors->get('password') as $message)
                <label class="control-label" for="inputError"><i class="fa fa-times-circle-o"></i>{{$message}}</label></br>
                @endforeach @endif

                <label class="form-label fw-bold">{{exmtrans('install.database.password')}}:</label>
