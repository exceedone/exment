...
class TableService
{
    ...
    public static function appendCreateAndDownloadButtonQRCode($tools, $custom_table)
    {
        ...
        $text_button_ja = $custom_table->getOption('text_button_ja');
        $text_button_en = $custom_table->getOption('text_button_en');
        if ($custom_table->getOption('active_qr_flg')) {
            $tools->append(new SwalInputButton([
                'url' => route('exment.qrcode_download', ['tableKey' => $custom_table->id]),
                'label' => $current_locale === 'ja'
                    ? ($text_button_ja ?? '二次元バーコード') . exmtrans("custom_table.qr_code.download")
                    : ($text_button_en ?? '2D barcode') . exmtrans("custom_table.qr_code.download"),
            ...
            $tools->append(new ModalButton([
                ...
                'label' => $current_locale === 'ja'
                    ? ($text_button_ja ?? '二次元バーコード') .exmtrans("custom_table.qr_code.create")
                    : exmtrans("custom_table.qr_code.create") . ($text_button_en ?? '2D barcode'),
                'icon' => 'fa-qrcode',
