...
class TenantService
{
    ...
    public function deleteTenant(string $tenantSuuid): array
    {
        try {
            ...
            // Only cleanup AWS resources for subdomain tenants
            if ($tenant->usesSubdomain() && $tenant->subdomain) {
                $baseDomain = Config::get('exment.tenant.base_domain');
                $fqdn = $tenant->subdomain . '.' . $baseDomain;

                // Delete Route53 record
                $route53Result = $this->route53Service->deleteSubdomainRecord($fqdn);
                if (!$route53Result['success']) {
                    Log::warning('Failed to delete Route53 record', [
                        'tenant_suuid' => $tenantSuuid,
                        'fqdn' => $fqdn,
                        'error' => $route53Result['message']
                    ]);
                }
            }

            // Mark tenant as deleted instead of physical deletion
     ...
     * @return Tenant|null
     */
    protected function getTenantById(string $tenantId): ?Tenant
    {
    ...
    public function updateTenant(string $tenantSuuid, array $input): array
    {
        try {
            // Find tenant
            $tenant = Tenant::where('tenant_suuid', $tenantSuuid)->first();
            if (!$tenant) {
                return [
                    'success' => false,
                    'error' => 'TENANT_NOT_FOUND',
    ...
    private function updateTenantWithSubdomainChange($tenant, array $input, array $oldPlanInfo, string $oldSubdomain): array
    {
        ...
        $tenant->update([
            ...
            'status' => TenantStatus::SUBDOMAIN_CHANGE_PENDING
        ]);
