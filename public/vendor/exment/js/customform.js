...
(function (Exment) {
    class CustomFromEvent {
        ...
        static loadingEvent() {
            // Add drag item event
            $('.custom_form_column_items .draggables').each(function (index, elem) {
         ...
         */
        static revertDeleteColumn($item, $clone) {
            if ($clone) {
                $clone.remove();
            }
        ...
        static validateOption(value, element) {
            ...
            // get rules
            let rules = JSON.parse($elem.closest('.custom_form_column_item').find('.validation_rules').val());
     ...
     * @param ev
     */
    CustomFromEvent.deleteColumnEvent = (ev) => {
        ev.preventDefault();
     ...
     */
    CustomFromEvent.isEnableResize = function (el, nextSize) {
        // calc size
